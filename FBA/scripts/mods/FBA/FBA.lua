...
--[[
	Hooks
--]]
mod:hook(PlayerUnitFirstPerson, "current_position", function (func, self, ...) --x
	local offset = (Managers.state.camera._camera_offset and Managers.state.camera._camera_offset:unbox()) or Vector3(0, 0, 0)
	return Unit.local_position(self.first_person_unit, 0) + offset
end)

mod:hook(PlayerUnitFirstPerson, "play_unit_sound_event", function (self, event, unit, node_id, play_on_husk, ...) --x
...
--	local PX,PY,PZ,PW = Quaternion.to_elements(pitch)
	Managers.state.camera:set_offset(0, (pitch * 0.14) + (0 * -1) / 100, 0)
